...
?>
	...
	<?php
		$score = 0;
	 $file = $_POST['extra_item'];
	 $orig_filesize = filesize(nifty_bu_upload_dir.$file);

	 ?>
<div id='nifty_dashboard_content'>
	<h3><?php _e("Restore (database)","nifty-backups"); ?></h3>
	...
	<p><?php _e("The length of time required to run the restore is dependant on the size of the SQL file that is being handled.","nifty-backups"); ?></p>
	<?php 
		if($safemode){ ?>
...
<?php } 
	 ...
	 if ($orig_filesize < 10000000) { $size_col = 'nifty-green'; $size_text = 'low risk of failure'; $score++; }
	 if ($orig_filesize >= 10000000 && $orig_filesize < 20000000) { $size_col = 'nifty-orange'; $size_text = 'medium risk of failure'; }
	 if ($orig_filesize >= 20000000) { $size_col = 'nifty-red'; $size_text = 'high risk of failure'; $score--; }
	...
	<p class='restore-buttons-p'>
		...
		<button id='nifty-button-restore-db_<?php echo $_POST['extra_item']; ?>' class='nifty-button-restore-db button button-primary' bid='<?php echo $_POST['extra_item'] ?>' value=''><span class='nifty-button-restore-db_span'><?php _e("Restore now","nifty-backups"); ?></span></button>
